...
void handler(uint8_t * args, const struct pcap_pkthdr *orig_header, 
    ...
    if (eth.ethtype == 0x0800) {
        ...
        // This code does not support other ethernet types so just return
        return;
    }
    ...
    if (ip.proto == 17) {
        ...
        // if it is not udp return as DNS is transported over UDP
        return;
    }
...
void print_summary(ip_info * ip, transport_info * trns, dns_info * dns,
    ...
    // Do not need to print transport protocol
    ...
    if (iptostr(&ip->dst) != NULL) { // check for null before printing
        printf("%s", iptostr(&ip->dst));
    }

    printf(",%d,%c,%s", dnslength, dns->qr ? 'r':'q', dns->AA?"AA":"NA");
