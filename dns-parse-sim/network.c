...
char IP_STR_BUFF[IP_STR_BUFF_SIZE];
...
uint32_t udp_parse(uint32_t pos, struct pcap_pkthdr *header, 
    ...
    // DNS is on port 53 and mDNS is on port 5353
    int is_dns = udp->srcport == 53 || udp->dstport == 53;

    //mDNS is on port 5353
    // bool is_mdns = udp->srcport == 5353 || udp->dstport == 5353;
